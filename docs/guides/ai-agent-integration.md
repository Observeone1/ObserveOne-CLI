...
description: How to configure autonomous artificial intelligence (AI) agents to use the ObserveOne CLI
---

# AI Agent Integration

The ObserveOne command-line interface (CLI) is explicitly designed to be driven by autonomous coding agents (such as Claude Code, Cursor, GitHub Copilot Workspaces, or custom bots). It features resilient machine-to-machine contracts and headless execution modes.
...
For `obs ai-check run`, JSON mode returns immediately with a task identifier unless `--wait` is supplied.
...
1. **Pull State:** Run `obs export --json` to fetch the user's current infrastructure into `obs.json`.
2. **Read & Edit:** Read `obs.json`, append the new required API Checks or URL Monitors to the arrays, and save the file.
