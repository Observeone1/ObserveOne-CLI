  ...
  if (result.exitCode === 0) {
    ...
    if (jsonStartIndex !== Infinity) {
      ...
      try {
        ...
        throw new Error(`JSON format: Output should contain valid JSON after progress indicators: ${jsonPart}`);
      }
    } else {
  ...
  if (result.exitCode === 0) {
    ...
    if (jsonStartIndex !== Infinity) {
      ...
      try {
        ...
        throw new Error(`Global JSON: Output should contain valid JSON after progress indicators: ${jsonPart}`);
      }
    } else {
      // If no JSON found, check if it's because we got an error instead
      if (output.toLowerCase().includes("error") || output.toLowerCase().includes("not found")) {
