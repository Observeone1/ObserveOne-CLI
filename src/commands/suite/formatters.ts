...
export function suiteStatusColor(status: string): string {
  switch (status) {
    case 'scheduled': return chalk.green(status);
    case 'failed': return chalk.red(status);
    case 'pending':
    case 'generating': return chalk.yellow(status);
    case 'crawling':
    case 'planning':
    case 'healing': return chalk.blue(status);
...
export function printExecutionResults(execution: SuiteExecution): void {
  ...
  const passed = execution.passed ?? execution.test_results.filter(r => r.status === 'PASSED').length;
  const failed = execution.failed ?? execution.test_results.filter(r => r.status === 'FAILED').length;
...
function printTestRow(r: SuiteTestResult): void {
  const icon =
    r.status === 'PASSED' ? chalk.green('PASSED ') :
    r.status === 'FAILED' ? chalk.red('FAILED ') :
    r.status === 'SKIPPED' ? chalk.yellow('SKIPPED') :
    chalk.gray('PENDING');
